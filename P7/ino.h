...
/**
 * This is taken from sys/ino.h of Unix Version 6.  The actual structure looked
 * like:
 *
 *   int    i_mode;
 *   char   i_nlink;
...
struct inode {
  ...
  uint16_t i_addr[8];  // block numbers for the file's data. For small
                       // files, these are the blocks of data. For
                       // large files these are indirect blocks,
                       // except for i_addr[7], which is a doubly
                       // indirect block. A block number of zero means
                       // "no such block".
